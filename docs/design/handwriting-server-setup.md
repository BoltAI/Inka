...
In debug builds, `Settings -> Developer -> Handwriting Lab -> Open` sends
sample text to the selected synthesis engine.

The lab sends sample text to the selected engine, renders the returned strokes,
and logs generation/render timing to logcat with the `HandwritingLab` tag.
...
curl -s http://127.0.0.1:8878/handwriting \
  ...
  -d '{
    "text":"again and again, a little different.",
    "pageWidth":1404,
    "pageHeight":1872,
    "left":96,
    "top":180,
