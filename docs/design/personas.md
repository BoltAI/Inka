...
Every request is composed as base rules plus one persona prompt. Base rules define the medium; personas define voice.
...
Whisper, Companion, Storyteller, Scholar, Muse, Socrates, Wit.

Custom persona text is appended after the base rules in the same shape as a built-in persona. Base rules always win on formatting and safety.
